...
// --- IPC HANDLERS PROTEGIDOS ---
// MongoDB
if (!ipcMain._mongoHandlersRegistered) {
...
if (!ipcMain._engineConfigRegistered) {
  ipcMain.handle('read-engine-config', async () => {
    try {
      const cfgPath = path.join(global.APP_ROOT, 'engine/config_local.json');
      const raw = fs.readFileSync(cfgPath, 'utf8');
...
// Notificaciones
if (!ipcMain._notificationsRegistered) {
  ...
  ipcMain._notificationsRegistered = true;
}
...
async function createWindow() {
  ...
  try {
    ...
    setupIpcHandlers(mainWindow); // IPC adicionales
  } catch (err) {
